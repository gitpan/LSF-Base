...
reference of array gives a list of hosts or clusters whose information is 
returned if they satisfy the requirements in resreq. if C<hosts> is undef, all 
...
better. if C<fromhost> is undef, the local host is assumed.
...
listsize host or cluster names. If not undef, only load information about 
...
task originates when LIM makes the placement decision. if C<fromhost> is undef,
...
C<resreq> is a resource requirement expression (which can be NULL) that 
...
character string containing the name of the task to be inesrted .
...
This routine initializes the LSF library for remote execution. C<options>
usually is set to 0.
...
with the application. C<hostname> is the name of target host. Returns 1 
means function was successful, 0 is failed.
...
environment, otherwise using the new one. C<argv> and C<options> are the same
...
support of setting up a new environment specified by the string array C<envp>.
When C<envp> is undef, it means using the remote RES server's cached
environment, otherwise using the new one. other arguments are the same as
...
This routine sllows an application program to query and specify how 
